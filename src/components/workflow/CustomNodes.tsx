...
import { Sparkles, Zap, Settings, GitBranch, ShieldCheck, Flag } from 'lucide-react';
...
export const TriggerNode = ({ data, selected }: WorkflowNodeProps) => (
...
export const ActionNode = ({ data, selected }: WorkflowNodeProps) => (
...
export const AINode = ({ data, selected }: WorkflowNodeProps) => (
...
export const ConditionNode = ({ data, selected }: WorkflowNodeProps) => (
  <div className={`${nodeBaseClass} border-sky-500/30 ${selected ? selectedClass : ''}`}>
    <div className="absolute top-0 left-0 w-1 h-full bg-sky-500" />
    <Handle type="target" position={Position.Left} className="!bg-sky-500 !w-2 !h-2 !border-none" />
    <div className="flex items-center gap-3 mb-2">
      <div className="w-8 h-8 rounded-lg bg-sky-500/10 flex items-center justify-center">
        <GitBranch className="w-4 h-4 text-sky-400" />
      </div>
      <div>
        <p className="text-[10px] font-bold text-sky-400 uppercase tracking-widest">Condition</p>
        <p className="text-sm font-bold text-white">{data.label}</p>
      </div>
    </div>
    <p className="text-[11px] text-slate-400 leading-relaxed truncate">{data.description}</p>
    <Handle type="source" position={Position.Right} className="!bg-sky-500 !w-2 !h-2 !border-none" />
  </div>
);

export const ApprovalNode = ({ data, selected }: WorkflowNodeProps) => (
  <div className={`${nodeBaseClass} border-orange-500/30 ${selected ? selectedClass : ''}`}>
    <div className="absolute top-0 left-0 w-1 h-full bg-orange-500" />
    <Handle type="target" position={Position.Left} className="!bg-orange-500 !w-2 !h-2 !border-none" />
    <div className="flex items-center gap-3 mb-2">
      <div className="w-8 h-8 rounded-lg bg-orange-500/10 flex items-center justify-center">
        <ShieldCheck className="w-4 h-4 text-orange-400" />
      </div>
      <div>
        <p className="text-[10px] font-bold text-orange-400 uppercase tracking-widest">Approval</p>
        <p className="text-sm font-bold text-white">{data.label}</p>
      </div>
    </div>
    <p className="text-[11px] text-slate-400 leading-relaxed truncate">{data.description}</p>
    <Handle type="source" position={Position.Right} className="!bg-orange-500 !w-2 !h-2 !border-none" />
  </div>
);

export const EndNode = ({ data, selected }: WorkflowNodeProps) => (
  <div className={`${nodeBaseClass} border-rose-500/30 ${selected ? selectedClass : ''}`}>
    <div className="absolute top-0 left-0 w-1 h-full bg-rose-500" />
    <Handle type="target" position={Position.Left} className="!bg-rose-500 !w-2 !h-2 !border-none" />
    <div className="flex items-center gap-3 mb-2">
      <div className="w-8 h-8 rounded-lg bg-rose-500/10 flex items-center justify-center">
        <Flag className="w-4 h-4 text-rose-400" />
      </div>
      <div>
        <p className="text-[10px] font-bold text-rose-400 uppercase tracking-widest">End</p>
        <p className="text-sm font-bold text-white">{data.label}</p>
      </div>
    </div>
    <p className="text-[11px] text-slate-400 leading-relaxed truncate">{data.description}</p>
